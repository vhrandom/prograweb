...
export function SearchBar({ onSearch, placeholder = "Buscar productos...", className = "" }: SearchBarProps) {
  ...
  // Sincronizar input con la URL (Si recargas la página o navegas atrás, el input se rellena solo)
  useEffect(() => {
    ...
    if (searchParam) {
      ...
      // Si no hay búsqueda en la URL, limpiamos el input (útil al volver al inicio)
      setQuery("");
    }
  }, [location]);
  ...
  return (
    <form onSubmit={handleSubmit} className={`relative ${className}`}>
      ...
      <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
        <kbd className="hidden sm:inline-flex items-center px-2 py-1 bg-white dark:bg-apple-dark-3 border border-apple-gray-3 dark:border-apple-dark-4 rounded text-caption-1 font-sans font-medium text-apple-gray-1">
